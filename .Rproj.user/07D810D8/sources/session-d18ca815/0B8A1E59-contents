test_that("extracting Barcode pairs works", {
  ...
  myBarbie <- Barbie::createBarbie(count)
  pairInfo <- extractBarcodePairs(myBarbie)
  ## confirm extracting all pairs
  expect_equal(nrow(pairInfo$corTestResults),
               choose(nrow(myBarbie$assay), 2))
  barcodeArray <- c(1:40, rep(41,10))
...
test_that("clustering Barcodes based on correlation works", {
  ...
  expect_equal(myBarbie$BarcodeCluster$corCluster, c(-1:-40, rep(1,10)),
               ignore_attr=TRUE)
  expect_message(
