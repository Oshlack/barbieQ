test_that("extracting count matrix, target file and factor colors works", {
  ...
  object3 <- createBarbie(object=barcodeCount, target=sampleConditions,
  ...
  expect_equal(object5$foo[[1]], 1:12, ignore_attr=TRUE)
  ## Updating a `Barbie` object by passing new `target` and `factorColors`
  object6 <- createBarbie(object=object4, target=data.frame(Mouse=rep(1:4, each=3)))
  ## success update sample conditions
  expect_equal(object6$metadata, data.frame(Mouse=rep(1:4, each=3)))
  object7 <- createBarbie(
    object=object4, target=data.frame(Mouse=rep(1:4, each=3)),
    factorColors=list(
      Mouse=c("1"="#111199", "2"="#112200", "3"="#441111", "4"="#000000")))
  ## success update color palettes
  expect_equal(object7$factorColors[[1]],
