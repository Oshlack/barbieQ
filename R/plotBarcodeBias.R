...
plotBarcodeBiasScatterPlot <- function(barbieQ, elementName = NULL, reorderRank = FALSE,
    ...
    p <- ggplot(mydata, aes(x = mydata[, xAxis], y = minusLogP, text = BarcodeID)) + geom_point(aes(color = direction,
        shape = direction, fill = direction), size = 4, stroke = 1) + theme_classic() +
        theme(aspect.ratio = 1) + labs(title = paste0(methodLs$aim, " : ", methodLs$contrastVector),
        y = "-log10(p.value)", x = xTitle[xAxis]) + geom_hline(yintercept = -log10(0.05),
        linetype = "dashed", color = "grey") + scale_color_manual(values = customColor) +
        scale_shape_manual(values = customShape) + scale_fill_manual(values = alpha(customColor,
        0.2))
