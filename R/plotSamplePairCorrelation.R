...
plotSamplePairCorrelation <- function(
    barbieQ, sampleOrder = NULL, targets = NULL, sampleGroups = NULL,
    method = "pearson") {
  ...
  topTargets <- mytargets[, dplyr::setdiff(seq_along(colnames(mytargets)), pointer),
    drop = FALSE
  ]
  ## check sampleOrder
  ...
  hp <- Heatmap(corMat,
    name = "correlation", width = unit(6, "cm"), height = unit(
      6,
      "cm"
    ), col = circlize::colorRamp2(c(-1, 0, 1), c("blue", "white", "red")), heatmap_legend_param = list(at = c(
      -1,
      0, 1
