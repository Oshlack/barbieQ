...
plotBarcodeProportion <- function(barbieQ, coordFixed = FALSE, colorGradient = FALSE) {
  ...
  data <- data.frame(
    individual = rownames(barbieQ$proportion), percentage = relativeContribution,
  ...
  suppressWarnings({
    p <- ggplot(data, aes(x = rank, y = percentage)) +
      geom_bar(
        stat = "identity",
        alpha = 1, color = alpha(barColor, 0.2), fill = barColor, linewidth = 1
      ) +
      labs(title = "Barcode Contribution", x = paste0(nrow(data), " Barcodes"), y = "Relative Average Barcode Proportion (%)") +
      theme_minimal() +
      theme(
